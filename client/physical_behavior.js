//Aircraft physics and stuff (called in script.js so it updates)
function aircraftPB() {
  let thrust = aircraft.throttle*localStorage.getItem('thrust');
  let drag = {
  ...
  aircraft.pitch = pitch * localStorage.getItem('pitch')/Math.PI;
  ...
  //detecting if aircraft can land
  if (aircraft.y <= 10) {
    let shock = 0;
