...
function draw() {
      ...
      for (let j = 0; j < object.faces.length; j++) {
        if (object.faces.length > 3) {
          ...
          if (cos < 0) {
            ...
            const oc = getRGB(object.faces[j][3]);
  ...
  ctx.font = "20px serif";
  ctx.fillStyle = "black";
  ctx.fillText(`Score: ${score}`,canvas.clientWidth/2,-canvas.clientHeight/2-100)
  ctx.fillText("fps: "+fps, -350, -(250))
  ctx.fillText(`Highscore: ${localStorage.getItem('highscore')}`,canvas.clientWidth/2,-canvas.clientHeight/2-75)
}
